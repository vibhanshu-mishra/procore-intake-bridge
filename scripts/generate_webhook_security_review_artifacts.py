...
def _generate(root: Path) -> int:
    result = write_webhook_security_review_artifacts(
        build_webhook_security_review_report(get_settings()), root
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate sanitized webhook review artifacts.")
    ...
    return _generate(args.output_root or settings.webhook_security_review_output_root)
