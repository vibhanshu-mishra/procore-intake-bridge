...
def _generate(root: Path) -> int:
    result = write_infra_security_review_artifacts(
        build_infra_security_review_report(get_settings()), root
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate sanitized infrastructure security review artifacts."
    )
    ...
    return _generate(args.output_root or settings.infra_security_review_output_root)
